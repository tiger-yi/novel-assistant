...
class TransactionCommitTest(unittest.TestCase):
    ...
    def _prepare_dialogue_clarity_transaction(self, audit_overrides=None):
        self.write_manifest(writes=["../chapters/"])
        manifest = yaml.safe_load(self.manifest_path.read_text(encoding="utf-8"))
        manifest["pipelines"]["update-world"]["stages"].insert(
            1,
            {
                ...
                "required": True,
                "allowed_statuses": ["PASS"],
                "required_evidence": ["dialogue_clarity_audit"],
            },
        )
        self.manifest_path.write_text(
            yaml.safe_dump(manifest, allow_unicode=True, sort_keys=False),
        ...
        staged = (
            "chapters/.staging/TX-CMD-UPDATE-WORLD-0001-R01/CH-0001.txt"
        )
        transaction_path = self.write_prepared_transaction(
            target="chapters/CH-0001.txt",
            staged=staged,
            baseline_hash="absent",
            staged_text="守门人道：\u201c路已经封了。\u201d\n",
        )
        audit = {
            "key": "dialogue_clarity_audit",
            "claim": "已完成关键台词风险扫描",
            "source": staged,
            "source_hash": sha256_file(self.root / staged),
            "scan_scope": {"start_line": 1, "end_line": 1},
            "risk_category_counts": {
                "character_decision": 0,
                "action_causality": 0,
                "fact_revelation": 0,
                "resource_or_state": 0,
                "relationship_commitment": 0,
                "payoff_or_closing_pull": 0,
            },
            "key_dialogues": [],
            "unresolved_items": [],
            "no_match_reason": "本句只报告眼前路况，不承担关键决定或章末牵引",
        }
        if audit_overrides:
    ...
    def test_accepts_dialogue_clarity_audit_with_explicit_zero_match_scan(self):
        transaction_path = self._prepare_dialogue_clarity_transaction()
    ...
    def test_rejects_dialogue_clarity_audit_bound_to_stale_chapter_hash(self):
        transaction_path = self._prepare_dialogue_clarity_transaction(
            {"source_hash": "sha256:" + "0" * 64}
        )

        with self.assertRaisesRegex(TransactionError, "source_hash"):
            commit_transaction(self.root, self.manifest_path, transaction_path)

    def test_rejects_passing_dialogue_clarity_audit_with_unresolved_items(self):
        transaction_path = self._prepare_dialogue_clarity_transaction(
            {"unresolved_items": [{"line": 1, "reason": "核心所指仍有两解"}]}
        )

        with self.assertRaisesRegex(TransactionError, "unresolved_items"):
            commit_transaction(self.root, self.manifest_path, transaction_path)

    def test_rejects_dialogue_clarity_excerpt_outside_declared_lines(self):
        transaction_path = self._prepare_dialogue_clarity_transaction(
            {
                "risk_category_counts": {
                    "character_decision": 1,
                    "action_causality": 0,
                    "fact_revelation": 0,
                    "resource_or_state": 0,
                    "relationship_commitment": 0,
                ...
                "key_dialogues": [
                    {
                        ...
                        "end_line": 1,
                        "excerpt": "并不存在的台词",
                        "risk_categories": ["character_decision"],
                        "core_referent": "守门人是否放行",
                        "character_stance": "拒绝放行",
                        "causal_role": "阻止来人继续前进",
                        "context_anchors": ["守门人报告道路封闭"],
                        "verdict": "clear",
                    }
                ],
        ...
        with self.assertRaisesRegex(TransactionError, "excerpt"):
            commit_transaction(self.root, self.manifest_path, transaction_path)
    ...
    def test_executor_rejects_reader_evaluation_report_with_stale_artifact_hash(self):
        transaction_path = self.write_prepared_transaction()
        self.stage_file(
            "world/.transactions/TX-CMD-UPDATE-WORLD-0001-R01/reader-evaluation-R1.md",
            "reader evaluation evidence\n",
    ...
    def test_executor_rejects_reader_cross_check_bound_to_stale_chapter_hash(self):
        self.write_manifest(writes=["../chapters/"])
        transaction_path = self.write_prepared_transaction(
            target="chapters/CH-0001.txt",
            staged="chapters/.staging/TX-CMD-UPDATE-WORLD-0001-R01/CH-0001.txt",
        ...
        report = {
            ...
            "dialogue_clarity_cross_check": {
                ...
                "audit_conflicts": [],
                "no_match_reason": "没有关键台词",
            },
        }
        report_path = self.stage_file(
            "world/.transactions/TX-CMD-UPDATE-WORLD-0001-R01/reader-evaluation-R1.md",
            "```yaml\n"
        ...
        transaction = load_transaction(transaction_path)
        transaction["reader_evaluation"] = {
            "artifact_path": report_path.relative_to(self.root).as_posix(),
            ...
            "commands": [
                {
                    "name": "reader-evaluation-contract",
                    "command": (
                        "python scripts/validate_reader_evaluation.py "
                        "<reader_evaluation_file>"
                    ),
                }
            ]
    ...
    def test_executor_rejects_reader_cross_check_that_skips_audited_key_dialogue(self):
        transaction_path = self._prepare_dialogue_clarity_transaction(
            {
                "risk_category_counts": {
                    "character_decision": 1,
                ...
                "key_dialogues": [
                    {
                        ...
                        "excerpt": "路已经封了",
                        "risk_categories": ["character_decision"],
                        "core_referent": "道路是否允许通行",
                        "character_stance": "拒绝放行",
                        "causal_role": "迫使来人停止前进",
                        "context_anchors": ["守门人明确报告道路封闭"],
                        "verdict": "clear",
                    }
                ],
                "no_match_reason": None,
        ...
        report = {
            "target_genre_reader": {
                "weighted_score": 8.0,
                "dimensions": [{"name": "翻页欲", "score": 8.0, "weight": 1.0}],
            },
            "dialogue_clarity_cross_check": {
            ...
            "commands": [
                {
                    "name": "reader-evaluation-contract",
                    "command": (
                        "python check_reader_evaluation.py <reader_evaluation_file>"
                    ),
                }
            ]
        }
        ...
        with self.assertRaisesRegex(TransactionError, "does not cover"):
            commit_transaction(self.root, self.manifest_path, transaction_path)
...
class TransactionBeginTest(unittest.TestCase):
    ...
    def test_migration_parent_adds_dialogue_clarity_issues_to_authorized_chapters(self):
        self.add_presentation_migration_routes()
        first = self.write_published_chapter(1, "守门人说今日不许通行。")
        second = self.write_published_chapter(2, "守门人道：\u201c这件事，我记着。\u201d")
        manifest = yaml.safe_load(self.manifest_path.read_text(encoding="utf-8"))
        scan_stage = manifest["pipelines"]["migration-scan"]["stages"][0]
        scan_stage["allowed_statuses"] = ["PASS"]
        scan_stage["required_evidence"] = ["dialogue_clarity_scan"]
        self.manifest_path.write_text(
            yaml.safe_dump(manifest, allow_unicode=True, sort_keys=False),
            encoding="utf-8",
        )
        ...
        parent = load_transaction(parent_path)
        self.assertEqual([], parent["migration"]["chapters"])
        parent["state"] = "PREPARED"
        parent["gates"] = [
            {
                "gate": "scan",
                ...
                "evidence": [
                    {
                        "key": "dialogue_clarity_scan",
                        "claim": "已扫描全部正式章节",
                        "source": "chapters/",
                        "scanned_chapters": [
                            {
                                "chapter": "CH-0001",
                                "source": first.relative_to(self.root).as_posix(),
                                "source_hash": sha256_file(first),
                            },
                            {
                                "chapter": "CH-0002",
                                "source": second.relative_to(self.root).as_posix(),
                                "source_hash": sha256_file(second),
                            },
                        ],
                        "issues": [
                            {
                                ...
                                "end_line": 1,
                                "excerpt": "这件事，我记着",
                                "risk_categories": ["payoff_or_closing_pull"],
                                "reason": "章末关键台词的核心所指无法唯一恢复",
                            }
                        ],
                        "unresolved_items": [],
                    }
        ...
        self.assertEqual(["CH-0002"], committed["migration"]["chapters"])
        self.assertEqual("AUTHORIZED", committed["migration"]["migration_state"])
