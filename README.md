...
已发布章节需要多次语言优化时使用 `润色章节 CH-0001`。该命令创建新的等义呈现修订事务，先做原文基线审计，再按当前 `writespec/style-guide.md` 把候选正文写入 staging；通过字数、格式、等义、风格应用、叙事完整性、世界观和线索门禁后，才由事务执行器覆盖正式章节。若只想处理当前活跃章节事务内的 staging，使用 `润色当前章节`；找不到活跃 staging 时返回 `STAGING_NOT_FOUND`。
