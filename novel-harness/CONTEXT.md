...
**爽点 (Reader Payoff)**:
读者承诺通过主角主动行动、可信代价、有利状态变化和可感确认得到回报的事件；不等于钩子、伏笔、单纯胜利或旁人震惊。
_Avoid_: 爆点、打脸、高潮 (只覆盖部分形态)

**微回报 (Micro Payoff)**:
改变当前场景局部优势的爽点，可以只持续到场景结束。
_Avoid_: 小爽点 (边界模糊)

**章节主爽点 (Chapter Primary Payoff)**:
完成本章核心读者承诺、且状态变化至少持续到下一章的主要爽点。
_Avoid_: 章末钩子、章节结果

**小高潮 (Short-Cycle Climax)**:
关闭约二至三章短冲突并形成至少两项持续状态变化的回报节点。
_Avoid_: 大爽点、普通微回报

**大爽点 (Major Payoff)**:
完成阶段目标并改变能力层级、身份、资源基础、核心关系或敌我格局的回报节点。
_Avoid_: 大场面、爆炸规模

**爽点密度档 (Payoff Density Profile)**:
约束章内回报下限、类型轮换和滚动高潮期限的 `standard` 或 `high` 策略；新书和卷开篇使用高密度档。
_Avoid_: 爽点数量、固定打脸次数

**爽点兑现证据 (Payoff Fulfillment Evidence)**:
正文对爽点承诺、行动、状态变化、确认、代价及滚动节奏的结构化定位证据。
_Avoid_: 爽点评分、Agent 口头判断
...
**签约首感风险**:
平台编辑或首批读者在前 500-1000 字、主要场景入口和章末牵引处, 因模板开局、工业化表达、人物功能化、桥段同质化或缺少本书独特指纹而产生的拒签、弃读或一眼 AI 判断风险；它服务正文首感和留存, 不等同于 AIGC 检测分数。
_Avoid_: 去 AI 味、检测器降分、普通读者评分
...
**自动化阻断分级**:
将章节工作流阻断拆为 `AUTO_FIX`、`AUTO_ROUTE_COMMIT`、`AUTO_ROUTE_REVIEW` 和 `HUMAN_REQUIRED` 四类；目标是在尽可能自动提交可验证低风险修复的同时，把改变冻结大纲、World Bible 核心事实、已发布事实或接受长期风险等不可逆决策保留给人工确认。
_Avoid_: 放宽门禁、跳过确认、无边界全自动提交
...
**关键台词**:
影响人物决定、行动因果、事实揭露、资源或状态、关系承诺、章节兑现或章末牵引的在场台词。
_Avoid_: 重要台词、高光台词

**关键台词语义闭合**:
关键台词的核心所指、人物立场和对后续因果的作用具有唯一可恢复的解释；次要双关、威胁程度和情绪深浅允许留白。普通含混先自动修复，修复后关键含义仍无法恢复才阻断。
_Avoid_: 符合人类阅读习惯、台词自然、读起来像人

**台词审查上下文**:
关键台词语义闭合所依据的完整本章，以及正文已明确承接的前章事实；前章事实可以提供来历，但关键台词的当前指向仍须在本章存在可识别锚点。
_Avoid_: 只看孤句、从全书任意寻找解释

**外部补释依赖**:
目标读者必须依赖台词审查上下文之外的补充说明，才能确定关键台词的核心所指、人物立场或因果作用。
_Avoid_: 需要作者解释、作者出来说明

**关键台词风险扫描**:
对完整章节中的人物决定、行动因果、事实揭露、资源或状态、关系承诺、章节兑现或章末牵引进行覆盖性扫描，并选出需要执行关键台词语义闭合审计的台词。
_Avoid_: 所有台词逐句精审、只记录发现的问题

**台词自然度**:
台词在口语节奏、人物声线、潜台词和信息表达方式上的质量表现；它可以自动优化和评分，但不因单纯生硬而构成不变量阻断。
_Avoid_: 关键台词语义闭合、人类感硬门禁
