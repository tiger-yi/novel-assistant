...
| IMP-0005 | 已晋升 | 用户明确要求关键人物台词优先自动识别和修复，并经 `grill-with-docs` 确认关键台词范围、完整本章加明确前情的审查上下文、核心语义唯一而次要留白、三轮等义自动修复、前瞻性流程范围和 INV 晋升。仓库审计证明 `reported_speech_audit` 只验证在场台词/转述是否合法，读者评价只有通用信息释放与台词失声扣分，执行器此前会放过过期正文 hash、未解决项和无法定位的短引；人工构造的事务、读者报告和历史扫描测试稳定复现缺口。 | 现有流程把“台词是否合法呈现”“台词是否自然”和“关键台词是否能由读者恢复核心含义”混在一起；缺少独立证据、最终正文绑定、读者交叉复核和语义历史扫描授权，导致 Agent 可凭创作意图自证清楚。 | 扩展 `INV-CHAPTER-001` 的关键台词语义闭合底线；四条正文输出 pipeline 的 `narrative-integrity` 强制 `dialogue_clarity_audit`，新章读者报告强制 `dialogue_clarity_cross_check`，显式历史扫描使用 `dialogue_clarity_scan`。语义由独立审计判断，执行器验证结构、hash、行号、短引、零命中理由和空未解决项；普通自然度仍归文本润色与评分。 | 已晋升为 `INV-CHAPTER-001` 语义规则，唯一权威仍为 `writespec/chapter-creation-spec.md`；同步 Manifest、润色/迁移命令、读者评价契约、执行器、README 与回归测试。 |
