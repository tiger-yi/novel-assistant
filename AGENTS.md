...
- **撰写阶段 TODO 示例**：
  - [ ] **环境与氛围**：设定本章感官锚点与干扰元素（如：雨腥味、地面的湿滑、远处忽远忽近的更夫声）。
  - [ ] **生理与限制**：确认角色当前的“即时生理负担”（如：旧伤导致的动作偏移、灵力枯竭的虚弱感）。
  - [ ] **信息差核对**：明确本场戏中各方角色的“视角边界”（严禁出现全知视角的降智行为）。
  - [ ] **动作与受力**：细化核心冲突中的物理反馈（灵力流转、法则碰撞、环境受力反馈如：踏碎青砖、震落积雪）。
  - [ ] **情绪侧写**：埋设 1-2 处路人震惊或反派心理博弈的侧写，强化叙事张力。
  - [ ] **去 AI 味审计**：预检并排除“总结性结尾”及 AI 高频词（如：显然、显而易见、璀璨夺目）。
  - [ ] **数据预告**：列出本章结束后需回写 `World Bible` 的数据变更（境界突破、道具消耗、时间推移）。

#### 2. 构思阶段 (Phase: Plan - ReAct)
参照规范:[logic-blueprint-spec.md](writespec/logic-blueprint-spec.md)
在完成 TODO 规划后，进入构思推演。
- **核心要求**：
  - **核心任务**：必须明确输出本章的核心任务（包含推进剧情、爽点设计、世界观展示、伏笔回收等维度。
  - 输出 **3-5 个核心剧情场景表格**（包含：场景、标题、核心动作、冲突/看点、情感/氛围）。
  - 在构思末尾，必须通过 `<ask_user_question>` 形式提供 **2-3 个剧情走向分支选项**，引导用户参与决策,**请告知您倾向的分支，我将立即开始执行 [撰写阶段] 的 TODO 规划。**。
  - **核心看点与伏笔预埋**：必须明确输出本章的核心吸引力及为后文埋下的线索。
...
- **输出要求**：
  ...
  - **命名规范**：严格采用 `XXXX-章节标题.md` 格式（XXXX 为四位数字编号，不足四位由 0 补齐，支持万章规模）。
  - **示例**：`0001-灭村之祸.md`、`0006-雷降药铺.md`。
- **过程要求**：
  - **显式输出**：必须显式输出 **Thought** 和 **Observation** 过程。
  - **核心审计**：Observation 部分若未提及对 `inventory.md` 的物品消耗审计和对 `power.md` 的战力压制核对，则该章节视为不合格，必须重写。
