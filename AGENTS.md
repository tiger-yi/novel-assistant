...
- 章节创作、润色、评价或回写过程中，如发现同类人工介入、重复补救、门禁冲突或规范空白，应记录证据并按 `continuous-improvement.md` 形成去重后的 `IMP-*` 候选，用于提升自动化流程、减少后续用户介入。
- 识别到适合晋升为 ADR 或 INV 的改进项时，只能提出分类建议、适用范围、阻断场景和验证方式；是否晋升及目标文件必须等待用户明确批准。
- 声称完成前必须执行相关确定性验证。
