...
- **构思阶段 TODO 示例**：
  ...
  - [ ] 明确本章“核心任务”（推进剧情/爽点设计/世界观展示/伏笔回收）。
  - [ ] 构思并输出 3-5 个核心剧情场景表格（场景/标题/动作/看点/氛围）。
  - [ ] 提出 2-3 个剧情走向分支建议。
  - [ ] 提炼本章“核心看点”与“伏笔预埋”。
...
在完成 TODO 规划后，进入构思推演。
- **核心要求**：
  - **核心任务**：必须明确输出本章的核心任务（包含推进剧情、爽点设计、世界观展示、伏笔回收等维度，参照 `构思优化1.md`）。
  - 输出 **3-5 个核心剧情场景表格**（包含：场景、标题、核心动作、冲突/看点、情感/氛围）。
  - 在构思末尾，必须通过 `<ask_user_question>` 形式提供 **2-3 个剧情走向分支选项**，引导用户参与决策。
  - **核心看点与伏笔预埋**：必须明确输出本章的核心吸引力及为后文埋下的线索（参照 `构思优化2.md`）。
  - **显式输出审计结果**：必须包含 **Observation (逻辑审计)** 环节，逐项列出校验结果（Pass/Fail）。
- **审计逻辑**：在 **Observation** 环节，如果未能通过“战力平衡”或“状态同步”审计，AI 必须显式写出：`[Audit Failed] -> Refine Plan`。
