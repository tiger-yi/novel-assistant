...
- **输入**: 指定 `审计原创性` 事务记录、用户选择的方案 A/B 或补充要求、故事力基线缺口、当前正式八类 World Bible。
- **前置条件**:
  - 八类 `world/*.md` 已由 `初始化世界` 创建。
  - `chapters/` 中不存在已发布 `CH-0001`。
  - 指定审计记录状态为 `COMPLETE`, 且包含风险项、返工验收条件、长篇展开性审计结论和故事力基线结论。
  - 同一目标不存在进行中的初始化返工事务。
...
2. **读取审计基线**: 提取风险项、方案 A/B、返工验收条件、长篇展开性审计结论、故事力基线结论和短期复查要求。
3. **生成返工候选版**: 可重写全部八类 World Bible, 包括 `outline.md`、人物、力量、地理、时间线、道具、叙事线索和章节摘要。候选必须把人物驱动、主题透镜、冲突引擎、反转承诺和角色功能落入对应 World Bible 文件。
4. **候选预览**: 输出与当前正式 World Bible 的差异摘要, 至少包含:
   ...
   - 故事力基线变化: 主角主动目标、主题透镜、有效冲突、情理内反转、角色功能和黄金三章牵引的变化。
