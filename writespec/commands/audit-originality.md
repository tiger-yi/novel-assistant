...
2. 按 `originality-audit.md` 完成全部维度审计，并引用文件、章节 ID 或实体 ID。第一章发布前必须额外输出故事力基线证据，证明 World Bible 已形成可写作的主角目标、主题、冲突、反转和角色功能。
3. 门禁结果统一使用 `PASS`、`WARN`、`FAIL` 或附原因的 `NOT_APPLICABLE`。
4. 审计报告必须输出可执行的返工验收条件；初始化后第一章发布前，必须额外执行长篇展开性审计和故事力基线审计。
...
第一章发布前, `审计原创性` 必须覆盖长篇展开性审计和故事力基线审计，并形成 `first-chapter-readiness` 结论:
...
第一章前审计出现以下任一情况时，至少判为 `WARN`；若多项同时缺失且无法通过候选返工补齐，判为 `FAIL`:

- 主角缺少主动目标、底层动机、可触发行动的缺陷或可验证弧光。
- 主题只停留在题材标签、爽点口号或设定介绍，不能约束卷目标和关键选择。
- ARC-001 缺少会改变资源、关系、信息、时间或势力格局的有效冲突。
- 反转只靠临时揭示或强行翻盘，缺少铺垫、误导、真实指向和回收范围。
- 配角、反派或势力只有功能标签，没有自身目标、资源、弱点和行动路线。
